...
const handleImageUpload = async (e) => {
    ...
    const fields = type === 'quest' ? [
        ...
        'Wildlife Adventure', 'Sports & Recreation'] },
...
const Admin = () => {
 ...
 useEffect(() => {
  ...
  const stats = useMemo(() => {
    ...
    // Per-user lifetime XP map (traveler_id -> Σ xp_value of approved quests).
    ...
    // Ecosystem Impact XP: the *lifetime* XP generated by approved quests.
    // This only grows — redemptions don't touch it because they deduct from
    // profiles.xp (the user's spendable wallet), not from the approved
    // submissions ledger. This is the metric that actually measures impact.
  ...
  const verifyMFAEnrollment = async () => {
    ...
    try {
        ...
        setTimeout(() => {
            window.location.replace(window.location.href); // Hardest possible reload
        }, 1000);
